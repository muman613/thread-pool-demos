  ...
  for (int i = 0; i < 100; i++) {
    pool.queue_job([]() -> bool {
        ...
        std::this_thread::sleep_for(
            std::chrono::microseconds(100 * (std::rand() % 10000)));
        return false;
    });
  }
}
...
int main(int argc, char *argv[]) {
  ...
  pool.queue_job(std::bind(do_download_url,
                           std::string("https://www.stackoverflow.com"),
                           std::string("stackoverflow.html")));
  pool.queue_job(std::bind(do_download_url,
                           std::string("https://www.foxnews.com"),
                           std::string("foxnews.html")));
  pool.queue_job(std::bind(do_download_url, std::string("https://www.cnn.com"),
                           std::string("cnn.html")));
